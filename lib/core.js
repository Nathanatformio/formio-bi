...
export const SOURCES = [
  ...
  { key: "linkedin_ads", label: "LinkedIn Ads", dsId: "LIA", supportsConversions: true,
    fields: ["date", "campaign_group", "cost", "impressions", "clicks", "conversions", "conversion_value"] },
  { key: "ga4", label: "Google Analytics 4", dsId: "GAWA", supportsConversions: true, isAnalytics: true,
    fields: ["date", "sessionDefaultChannelGroup", "sessions", "conversions", "totalRevenue"] },
  { key: "microsoft_ads", label: "Microsoft Ads", dsId: "AC", supportsConversions: true,
    fields: ["date", "campaign", "spend", "impressions", "clicks", "conversions", "revenue"] },
  { key: "chatgpt_ads", label: "ChatGPT Ads", dsId: "CGPTA", supportsConversions: false,
...
/* =============================== STORE ================================ */
// Production: Vercel Blob (detected via BLOB_STORE_ID, auth via OIDC — no static
// token needed). Local dev: filesystem fallback.
const DIR = path.join(process.cwd(), "data", "snapshots");
const LATEST_BLOB = "report-latest.json";
...
export async function saveSnapshot(report) {
  if (useBlob()) {
    ...
    const opts = { access: "public", contentType: "application/json", allowOverwrite: true, addRandomSuffix: false };
    await put(`report-${report.weekOf}.json`, body, opts);
    await put(LATEST_BLOB, body, opts);
    return;
  }
...
export async function getLatest() {
  if (useBlob()) {
    ...
    const { blobs } = await list({ prefix: LATEST_BLOB });
